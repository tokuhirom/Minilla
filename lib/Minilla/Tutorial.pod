...
If this is your first conversion to Minilla and want to make sure you're not going to mess CPAN with a bad archive when something goes wrong, you can run the releas command with FAKE_RELEASE environment variable. This will run all the other release process, except the UploadToCPAN step.
...
and if it was successful, you get a build in a directory called Dist-Name-v0.1.0 under your current directory. They can be later removed with C<minil clean> command.
